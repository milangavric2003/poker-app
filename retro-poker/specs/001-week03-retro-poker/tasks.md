...
A = ti, engine/backend/botovi. B = kolega, evaluator/frontend/demo.
...
Checkpoint: domen US2 prolazi. B može review-ovati potove dok A review-uje evaluator,
...
- [ ] T020 [US1] B — RED u tests/ui/table.test.tsx, tests/ui/actions.test.tsx i tests/ui/api.test.ts za početak, karte/stack/pot, legalne kontrole i amountTo/doplatu, loading blokadu, schema odgovora, potvrdu reseta; deps: T004, T019; dokaz: validni javni fixtures bez ručno dupliranih poker pravila.
- [ ] T021 [US1] B — GREEN u frontend/src/main.tsx, frontend/src/App.tsx, frontend/src/api.ts, frontend/src/components/Table.tsx, frontend/src/components/ActionPanel.tsx, frontend/src/components/HandResult.tsx i frontend/src/styles.css; deps: T020; dokaz: lokalni kompletan potez, događaji redom, rezultat ostaje vidljiv.
...
- [ ] T023 [US1] A+B — GREEN integracionih razlika samo u backend/src/session.ts, backend/src/routes.ts, backend/src/view.ts, frontend/src/App.tsx i frontend/src/api.ts; deps: T022; dokaz: E2E i postojeća regresija. Ako test odmah prođe, zabeležiti proveru postojećeg ponašanja bez izmišljanja RED-a ili promene koda.
...
- [ ] T025 [US3] A — RED u tests/unit/positions.test.ts za sve BL01–BL10 i rotacije, kratke blindove i neiskorišćena mesta; deps: T024; dokaz: AC21 konkretne pozicije, pre/post actor i izostanak uzastopnog BB u heads-up prelazu.
- [ ] T026 [US3] A — GREEN prelazi u backend/src/engine/positions.ts; Seat „Ceo broj 0–5”, stabilan configured ring, dead SB bez naplate i heads-up izuzetak; deps: T025; dokaz: sve BL tabele i početni AC02 prolaze.
- [ ] T027 [US3] A — RED u tests/integration/session.test.ts za next-hand, „status playing/won/lost”, AC20, stack carry-over, odbijen prerani/dupli nastavak, FR-018 rotaciju istorije i reset; deps: T026; dokaz: ljudska eliminacija zaustavlja session čak i kada čisti positions modul može računati botove.
- [ ] T028 [US3] A — GREEN u backend/src/session.ts, backend/src/routes.ts, backend/src/engine/hand.ts i backend/src/engine/history.ts za next-hand i ispadanje tek posle potova; deps: T027; dokaz: AC20/AC21 i gubitak memorije pri novom app procesu.
...
- [ ] T031 [US4] A — Proširiti RED tests/contract/view.test.ts i tests/integration/concurrency.test.ts: AC19 kroz sve faze/fold/showdown, svaki nested event/result bez tajni, AC18, create/reset preconditions, 400/404/409/413/415/428/500 i rollback RNG; deps: T030; dokaz: ulazi iz celog HTTP ugovora.
- [ ] T032 [US4] A — GREEN potrebnih granica u backend/src/view.ts, backend/src/session.ts, backend/src/routes.ts i backend/src/app.ts; deps: T031; dokaz: stale/double ne mutira, samo dozvoljeni Origin, bez debug endpoint-a. Već prolazne slučajeve beležiti kao regresiju, ne fabrikovati RED.
...
## Parallel Opportunities (ljudski rad, jedan agent)

- US2: posle T005, B vodi evaluator T006–T007, A wagering T008–T009 u različitim fajlovima.
- US1: B može pripremiti UI test slučajeve iz shared fixtures dok A vodi rute;
  stvarna integracija T021–T023 čeka backend i ugovore.
- US3: A računa positions oracle, B priprema result/next-hand UI scenarije;
  implementacija po navedenim zavisnostima.
- US4: A pregledava javni serializer, B pregledava fokus i layout.
  Shared ugovori i logovi integrišu se serijski.
