...
export function App() {
  ...
  return <div className="app-shell"><header className="titlebar"><h1>RETRO POKER</h1><span>LOCAL TABLE // NO LIMIT</span></header>
    ...
    {game && <><Table game={game} />{game.legalActions.length > 0 && <ActionPanel game={game} pending={pending} onAction={act} />}{visibleResult && <HandResult result={visibleResult} />}{game.phase === 'complete' && game.status === 'playing' && <button disabled={pending} onClick={advance}>Sledeća ruka</button>}</>}
  </div>;
}
