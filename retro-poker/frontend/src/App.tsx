...
import { createGame, loadGame, nextHand, sendAction, type ActionDraft } from './api';
...
export function App() {
  ...
  async function synchronize() {
    if (pending) return;
    setPending(true);
    try { setGame(await loadGame()); setError(null); }
  ...
  async function act(action: ActionDraft) {
    if (!game || pending || error) return; setPending(true); setError(null);
  ...
  async function advance() {
    if (!game || pending || error) return; setPending(true); setError(null);
    try { setGame(await nextHand(game)); }
  ...
  return <div className="app-shell"><header className="titlebar"><h1>RETRO POKER</h1><span>LOCAL TABLE // NO LIMIT</span></header>
    <section className="new-game" aria-label="Nova partija"><label htmlFor="bot-count">Broj botova</label><select id="bot-count" value={botCount} disabled={pending} onChange={e => setBotCount(Number(e.target.value))}>{[1, 2, 3, 4, 5].map(count => <option key={count}>{count}</option>)}</select><button disabled={pending || !!error} onClick={start}>Nova partija</button></section>
    {error && <><p className="error" role="alert">{error} Učitaj stanje pre nastavka.</p><button disabled={pending} onClick={synchronize}>Učitaj stanje</button></>}{!game && !pending && !error && <p className="empty">Izaberi broj botova i pokreni lokalnu partiju.</p>}
    {game && <><Table game={game} />{game.legalActions.length > 0 && <ActionPanel game={game} pending={pending || !!error} onAction={act} />}{visibleResult && <HandResult result={visibleResult} />}{game.phase === 'complete' && game.status === 'playing' && <button disabled={pending || !!error} onClick={advance}>Sledeća ruka</button>}</>}
  </div>;
}
