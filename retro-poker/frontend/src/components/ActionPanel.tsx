...
export function ActionPanel({ game, pending, onAction }: Props) {
  ...
  return <section className="actions" aria-label="Akcije">
    <h2>Tvoj potez <span className="action-hint">— izaberi legalnu akciju</span></h2>
    ...
    {ranged && <div className="raise-control">
      ...
      <input id="amount-to" aria-describedby="amount-help" type="number" min={ranged.minAmountTo} max={ranged.maxAmountTo} value={amountTo} disabled={pending} onChange={event => setAmountTo(Number(event.target.value))} />
      <span id="amount-help"><span>Doplata sa stacka: {Math.max(0, amountTo - contribution)}</span><small> · dozvoljeno {ranged.minAmountTo}–{ranged.maxAmountTo}</small></span>
